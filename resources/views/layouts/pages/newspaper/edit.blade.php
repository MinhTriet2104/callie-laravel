...
<body>
<div class="container">
  <h1 class="text-center my-3 mt-3">Update Newspaper</h1>
  <!-- Success -->
  ...
  @endif
  <!-- Form -->
  <form action="{{ route('newspaper.update', $newspaper->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    {{ method_field('PATCH') }}
    <div class="form-row">
      <div class="form-group col-md-6">
        <label for="title">News Title</label>
        <input type="text" class="form-control" id="title" name="title" placeholder="Title" value="{{ $newspaper->newspaper_title }}">
      </div>
      <div class="form-group col-md-6">
        <label for="author">Author</label>
        <select id="author" name="author" class="form-control">
          @foreach ($authors as $author)
            @if ($author->id == $newspaper->author_id)
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-6">
        ...
        <input type="hidden" name="old_news_img" value="{{ $newspaper->newspaper_imgae }}">
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="from-group col-md-12">
        ...
        <textarea class="form-control" id="summernote" name="content">{{ $newspaper->newspaper_content }}</textarea>
      </div>
    </div>
    <button type="submit" class="btn btn-primary btn-block py-2 mt-4">Update</button>
  </form>
</div>
<script>
